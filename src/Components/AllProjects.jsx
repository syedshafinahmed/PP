...
const AllProjects = () => {
  ...
  return (
    <section className="relative py-20 px-6 max-w-7xl mx-auto min-h-screen">
      <div className="relative z-10">
        {/* Header */}
        <div className="flex flex-col md:flex-row gap-5 md:gap-0 items-center justify-between mt-10 md:mt-20 mb-10 md:mb-20">
          <motion.button
            whileHover={reduceMotion ? {} : { scale: 1.05, x: -5 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => navigate(-1)}
        ...
        {/* Error */}
        {error && (
          <div className="text-center text-red-400 mb-10">{error}</div>
        )}
                    ...
                    {project.technologies?.slice(0, 4).map((tech) => (
                      ...
                      >
                        ...
                        <span className="absolute -top-10 left-1/2 -translate-x-1/2 w-max px-2 py-1 text-xs bg-black/90 backdrop-blur-sm rounded-md text-white opacity-0 group-hover/tech:opacity-100 transition-opacity duration-300 pointer-events-none whitespace-nowrap z-30">
                          {tech}
                          <div className="absolute top-full left-1/2 -translate-x-1/2 w-0 h-0 border-l-3 border-r-3 border-t-3 border-transparent border-t-black/90" />
                        </span>
                      </span>
                    ))}
                  </div>
